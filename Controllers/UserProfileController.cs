...
[HttpGet("{id}/orders")]
[Authorize]
 public IActionResult GetWithPastOrders(int id)
    {
        UserProfile foundUser = _dbContext.UserProfiles
        .Include(up => up.Orders)
        .ThenInclude(s => s.Status)
        ...
        .Select(up => new UserProfile
        {
            ...
            }).OrderByDescending(o => o.OrderDate).Where(o => o.OrderDate != null).ToList()
        })
        .SingleOrDefault(up => up.Id == id);
 ...
 [HttpPut("{id}")]
    ...
    public IActionResult UpdateUser(int id, UserProfile userProfile)
    {
        ...
        foundUser.IdentityUser.Email = userProfile.Email;
        foundUser.PhoneNumber = userProfile.PhoneNumber;
        foundUser.FirstName = userProfile.FirstName;
        foundUser.LastName = userProfile.LastName;

        // _dbContext.UserProfiles.Update(foundUser);
